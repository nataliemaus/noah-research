...
# create latent data using vanilla JTVAE instead of triplet!! 
    #test regular training w/ VANILLA JTVAE
...
if make_y_dec: 
    ...
    y_df.to_csv(train_data_folder  + 'train_y_dec_' + str(spz) + 'spz.csv', header=None, index = None)
    sys.exit()
...
with torch.no_grad():
    ...
    z_np = train_z.numpy() 
    z_df = pd.DataFrame(z_np)
    z_df.to_csv('train_data_vanilla/train_z.csv', header=None, index = None)

print("train_z vailla saved!")
logPs = []
for smile in train_x:
    logPs.append(penalized_logP(smile)) 
...
print("train_y_shape", y_np.shape)
y_df = pd.DataFrame(y_np)
y_df.to_csv('train_data_vanilla/train_y.csv', header=None, index = None)
# may have to edit this so y's are 
# from DECODED z vecs like before!
# do both and compare results?
